...
import { LogIn, Lock, Mail, PlayCircle, Database } from 'lucide-react';
...
export const LoginScreen: React.FC<LoginScreenProps> = ({
  ...
  const handleOfflineLogin = () => {
    localStorage.setItem('mp_user_email', 'admin@mpcargas.com.br');
    localStorage.setItem('mp_user_role', 'admin');
    onLoginSuccess('admin@mpcargas.com.br');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      // Se não há supabase configurado, entra automaticamente em modo offline
      handleOfflineLogin();
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (err.message?.includes('Invalid login credentials')) {
        ...
        setErrorMessage('Supabase offline ou sem conexão. Você pode entrar pelo Modo Offline abaixo.');
      } else {
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 p-8 sm:p-10">
          ...
          {/* Botão Destaque: Modo Offline / Teste */}
          <div className="mb-6 p-4 bg-amber-50/80 border-2 border-[#FFD100] rounded-2xl text-center">
            <p className="text-xs font-bold text-gray-800 mb-2">
              🚀 Acesso Rápido para Desenvolvimento e Testes
            </p>
            <button
              type="button"
              onClick={handleOfflineLogin}
              className="w-full bg-[#111111] hover:bg-black text-[#FFD100] font-black text-sm py-3 px-4 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-95"
            >
              <PlayCircle className="w-5 h-5 text-[#FFD100]" />
              ENTRAR EM MODO OFFLINE / TESTE
            </button>
          </div>
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  {isSignUp ? 'Criar Conta e Entrar' : 'ENTRAR VIA BANCO'}
                </>
              )}
            </button>
          </form>

          {/* Alternador Entre Login e Cadastro & Configuração */}
          <div className="mt-6 text-center pt-4 border-t border-gray-100 flex flex-col gap-2">
            ...
            <button
              ...
              className="text-xs font-bold text-gray-500 hover:text-black flex items-center justify-center gap-1.5 py-1"
            >
